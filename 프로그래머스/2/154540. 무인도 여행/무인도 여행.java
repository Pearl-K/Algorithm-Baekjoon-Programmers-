        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                if (!vst[row][col] && maps[row].charAt(col) != WATER) {
                    int sum = exploreIsland(row, col, maps, vst);
                    islandSums.add(sum);
                }
            }
        }

        if (islandSums.isEmpty()) return new int[] { -1 };
        
        return islandSums.stream()
                         .sorted()
                         .mapToInt(Integer::intValue)
                         .toArray();
    }
    
    private int exploreIsland(int stRow, int stCol, String[] maps, boolean[][] vst) {
        int total = 0;
